...
# Import the working enhanced RAG system
try:
    from working_enhanced_rag import WorkingRAGSystem
except ImportError as e:
    st.error(f"Error importing WorkingRAGSystem: {e}")
    st.error("Make sure working_enhanced_rag.py is in the same directory")
    st.stop()
...
def initialize_rag():
    """Initialize the working enhanced RAG system silently"""
    try:
        GROQ_API_KEY = "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"

        # Create the working RAG system
        st.session_state.rag_system = WorkingRAGSystem(GROQ_API_KEY)

        # Setup the system with hybrid data
        st.session_state.rag_system.setup_system(force_reload=False)
        
        # Get ChromaDB count
        try:
            st.session_state.chroma_count = st.session_state.rag_system.chroma_manager.collection.count()
        except Exception as e:
            st.session_state.chroma_count = 0

        st.session_state.initialized = True
        st.session_state.embedding_model = st.session_state.rag_system.chroma_manager.current_model
...
def process_query(query):
    """Process a user query using the working RAG system"""
    if not st.session_state.rag_system:
        return {"error": "RAG system not ready"}
    
    try:
        start_time = time.time()
        # Use the working RAG system process_query method
        result = st.session_state.rag_system.process_query(query)
        
        # Execute the SQL query
        sql_data, sql_success = st.session_state.rag_system.execute_query(result.enhanced_sql)
        processing_time = time.time() - start_time

        # Generate LLM response for display
        llm_response = f"Query processed using {result.method}. "
        llm_response += f"Similarity score: {result.similarity:.3f}. "
        llm_response += f"Retrieved {len(sql_data)} records from the ARGO database."

        response = {
            "query": query,
            "processing_time": processing_time,
            "best_match_id": "working_rag",
            "similarity": result.similarity,
            "matched_sample": result.metadata,
            "llm_response": llm_response,
            "sql_executed": sql_success,
            "sql": result.enhanced_sql,
            "sql_data": sql_data,
            "visualization_created": False,  # Can be enhanced later
            "plot_type": "",
            "plot_html": ""
        }
        return response
...
def get_float_trajectory(float_id):
    ...
    try:
        ...
        df = rag_system.query_engine.execute(sql_query).fetchdf()
...
def create_enhanced_float_map():
    ...
    # First, create the filters dropdown above the map title
    st.markdown("### Global ARGO Float Distribution Map")
    
    # Filters dropdown above the map
    with st.expander("🌊 Filters & Controls", expanded=False):
        ...
        with col1:
            if st.button("Show Trajectory", use_container_width=True, type="secondary"):
                if float_id_input and float_id_input.strip():
        ...
        with st.spinner("Loading ARGO float locations..."):
            ...
            if rag_system.query_engine:
                ...
                if not df.empty:
                    ...
                    # Create folium map with satellite as the default basemap
                    m = folium.Map(
                    ...
                    # Add markers for each float (hidden when trajectory is shown)
                    show_all_markers = not (st.session_state.get('show_trajectory', False) and st.session_state.get('trajectory_float_id'))
                    for idx, row in (latest_locations.iterrows() if show_all_markers else []):
        ...
        with st.spinner("Loading ARGO float locations..."):
            ...
            if rag_system.query_engine:
                ...
                if not df.empty:
                    ...
                    # Create folium map (satellite by default)
                    m = folium.Map(
                        location=[center_lat, center_lon],
                        zoom_start=3,
                        width='100%',
...
def main():
    ...
    with main_col2:
        ...
        # Wide input area (full container width)
        query = st.text_area(
            "Scientific Query",
            value=preset_query,
            height=130,
        ...
        # Buttons row directly below input – equal widths and spacing
        btn_col1, btn_col2, btn_col3 = st.columns(3)
        with btn_col1:
            submit_btn = st.button("Analyze", type="primary", use_container_width=True, key="analyze_btn")
        with btn_col2:
            clear_btn = st.button("Clear Results", use_container_width=True, key="clear_btn", disabled=st.session_state.query_results is None)
        with btn_col3:
            examples_btn = st.button("Examples", use_container_width=True, key="examples_btn")
